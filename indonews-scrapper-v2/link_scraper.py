...
def main():
    ...
    """
    import os
    if not os.path.exists('dataset_urls.csv'):
        logging.error("File 'dataset_urls.csv' tidak ditemukan. Pastikan file sudah dibuat.")
        return

    all_article_links = pd.read_csv('dataset_urls.csv', header=None)[0].dropna().unique().tolist()
    ...
    # Simpan URL gagal ke file
    if failed_urls:
        failed_filename = f"failed_urls_{today_date}.txt"
        with open(failed_filename, "w", encoding="utf-8") as f:
            for url in failed_urls:
                f.write(url + "\n")
        print(f"❌ {len(failed_urls)} artikel gagal di-scrape. Daftar URL gagal disimpan di '{failed_filename}'.")
